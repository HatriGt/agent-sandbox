 ...
 * checkpoints live under /workspace/.agent.ckpt/ in the box's own disk and die with the box.
 *
...
export const CKPT_DIR = "/workspace/.agent.ckpt";
...
/**
 * Capture checkpoint t<n>: tar workspace + agent memory into the checkpoint dir, then prune the
 * ring. The tar EXCLUDES the checkpoint dir itself (or each capture would swallow its ancestors).
 * `--warning=no-file-changed` + trailing `|| [ $? -eq 1 ]`: tar exits 1 for "file changed as we
 * read it", which between turns can only be the log tail — content-harmless, must not fail capture.
 */
export function captureCmd(n: number): string {
  ...
  return (
    ...
    `--exclude=${q(CKPT_DIR)} --exclude='${CKPT_DIR.slice(1)}' ` +
...
/**
 * Restore checkpoint t<n> in place: move the checkpoint store OUT of /workspace first (so the wipe
 * can't destroy the very tar being restored — and later checkpoints stay available for redo until
 * the next capture prunes them), wipe workspace + agent home, untar, move the store back, then
 * append an honest seam to the restored log so history shows the rewrite.
 */
export function revertCmd(n: number, discarded: number): string {
  const hold = "/tmp/.agent.ckpt.hold";
  ...
  return (
    ...
    `rm -rf ${q(hold)} && mv ${q(CKPT_DIR)} ${q(hold)} && ` +
    `rm -rf ${WORKSPACE}/* ${WORKSPACE}/.[!.]* ${AGENT_HOME} 2>/dev/null; ` +
    `tar -xf ${q(`${hold}/t${n}.tar`)} -C / && ` +
    `mv ${q(hold)} ${q(CKPT_DIR)} && ` +
